...
const getPuzzleAsynAwait = async (wordCount) => {
...
const getCountryAsyncAwait = async () => {
  const response = await fetch(
    "https://ipinfo.io/json?token=30215cf9b60933",
    {}
  );
...
const getCountryFlagAsyncAwait = async (countryCode) => {
  const response = await fetch("https://restcountries.com/v3.1/all", {});

  if (response.status === 200) {
    const data = await response.json();
    const matchedCountry = data.filter((country) => {
      return country.cca2 === countryCode;
    });
    return matchedCountry[0]["name"]["official"];
  } else {
    throw new Error("Something went wrong");
  }
};
...
const getCurrentCountry = ()=>{
  //Get the current country based on the Ip address
  // Use the country 
